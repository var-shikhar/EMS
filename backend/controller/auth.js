...
const postStudentLogin = async (req, res) => {
    ...
    try {
        const foundStudent = await StudentRole.findOne({ uniqueID: studentID });
        if(!foundStudent){
            res.status(RouteCode.NOT_FOUND.statusCode).json({ message: 'Student not found, Try again!' });
        }

        let foundUser = await User.findById(foundStudent.studentID);
        if (!foundUser) {
            res.status(RouteCode.NOT_FOUND.statusCode).json({message: 'Account not found, Try creating a new one!' });
        }

        // Check if password matches
        const isCorrectPassowrd = await bcrypt.compare(password.toString(), foundUser.password);
        if (!isCorrectPassowrd) {
            res.status(RouteCode.UNAUTHORIZED.statusCode).json({ message: 'Password is incorrect' });
        }

        // Generating and Setting a JWT token in cookie with domain and path options
        const jwtToken = jwt.sign({ email: foundUser.userEmail }, JWT_SECRET_KEY, { expiresIn: '1d' });
        res.cookie('tkn', jwtToken, { secure: true, httpOnly: true, sameSite: 'None'});
        res.status(RouteCode.SUCCESS.statusCode).json(foundUser._id);
    } catch (err) {
        console.error(err);
        res.status(RouteCode.SERVER_ERROR.statusCode).json({ message: RouteCode.SERVER_ERROR.message })
    }
...
export default {
    postRegister,
    postLogin, postStudentLogin, getLogout,
